...
/*	This copy has been edited from the version used by WmDOT
 */
...
//	AILog.Info("     Connecting " + AITown.GetName(ConnectPairs[0]) + " and " + AITown.GetName(ConnectPairs[1]) + "...");
  ...
  local pathfinder = RoadPathFinder();
	...
	//	Set Parameters
/*	pathfinder.cost.max_bridge_length = 6;
	pathfinder.cost.max_tunnel_length = 6;
...
$/	
  ...
  pathfinder.InitializePath([ConnectPairs[0]], [ConnectPairs[1]]);

  /* Try to find a path. */
//	AILog.Info("          Pathfinding from " + AIMap.GetTileX(ConnectPairs[0]) + "," + AIMap.GetTileX(ConnectPairs[0])+ " to " + AIMap.GetTileX(ConnectPairs[1]) + "," + AIMap.GetTileX(ConnectPairs[0])+ "...");
  local path = false;
  while (path == false) {
...
//	AILog.Info("          Route complete. (MD = " + AIMap.DistanceManhattan(ConnectPairs[0], ConnectPairs[1]) + ") Took " + (this.GetTick() - tick) + " tick(s)."); 
 }